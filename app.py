...
def generate_pauses():
    ...
    html += "<br><div class='no-print' style='text-align:center'><button onclick='window.print()' style='padding:10px 20px; font-weight:bold; cursor:pointer;'>🖨️ IMPRIMER LA FEUILLE</button></div></body></html>"
...
def generate_planning():
    ...
    <style>
        ...
        .main-container {{ background: white; border-radius: 12px; box-shadow: 0 10px 30px rgba(0,0,0,0.05); padding: 20px; margin: 10px auto; width: 96%; box-sizing: border-box; }}
...
def get_archives():
    try:
        fichiers = sorted([f for f in os.listdir(PLANNINGS_DIR) if f.startswith("Planning_A4_") and f.endswith(".html")], reverse=True)
        return jsonify([{'filename': f, 'name': f.replace("Planning_A4_", "").replace(".html", "").replace("-", "/")} for f in fichiers])
